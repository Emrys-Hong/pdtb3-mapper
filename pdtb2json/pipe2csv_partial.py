...
def correct_arg_span(parse_dict, pdtb3):
    ...
    for i in range(len(pdtb3)):
        if pdtb3.loc[i,'DocID'][4:6] not in ['00', '01', '24']: continue
        if i%100==0:print(i); print(pdtb3.loc[i,'DocID'])
...
if __name__ == "__main__":
    df = pd.read_csv('pdtb3_ordered.csv')
...
#     parse_dict = merge3dicts(parse_dict_train, parse_dict_dev, parse_dict_test)
    ...
    parse_dict = json.loads(codecs.open('/home/pengfei/data/pdtb3-dataset/all/conll/all/pdtb-parses.json').read())
    df = correct_arg_span(parse_dict, df)
    df.to_csv('pdtb3_ordered.csv', index=False)
